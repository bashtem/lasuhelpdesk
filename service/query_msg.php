...
$per_page=20;
$sql_count="SELECT * FROM query_msg WHERE unit_id='$unit_id' AND (recipient_view!='' OR recipient_time!='')";
...
$sql=$sql_count."ORDER BY sender_time DESC LIMIT $start,$per_page ";
...
$reqId = $row['req_id'];
	...
	$recipientView = $row['recipient_view'];
	...
	$response .='\r\r'.date("h:i:s A",$responseTime).'\r'.date("d/m/y",$responseTime); 
...
if(($recipientView==1) || ($request_status==0))
	$rowcolor="	#DBAAAA";
else
	$rowcolor="";
...
echo "<td><a href='#'  style='text-decoration:none; font-size: 13px'  onclick=\"recipient('$response','$req_id','$senderMsg','$responseTime')\" >View / Reply <span  class=\"glyphicon glyphicon-comment\"></span></a></td>";
echo "</tr>";
...
<ul  class="pagination pagination-sm">
		...
		echo	"
        <li><a href=\"#\"  onclick=\"pages('query_msg.php',$page_no-1,'');\">Previous</a></li>";		
		}
		for($x=1;$x<=$pages;$x++){
		echo ($page_no==$x)? "<li style='color:green'><a   href='#' onclick=\"pages('query_msg.php','$x','');\" ><b style=\"color:red\">$x</b></a></li>" : "<li><a href='#' onclick=\"pages('query_msg.php','$x','');\" >$x</a></li>";
		}
		if($page_no<$pages){
		echo "<li><a href=\"#\"  onclick=\"pages('query_msg.php',$page_no+1,'');\"  >Next</a></li>";
		}
