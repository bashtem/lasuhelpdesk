...
include("../../connect.php");
	...
	$sql="SELECT * FROM unit_head where email='$email' OR pfnumber='$pfno' ";	
	...
	if (mysqli_num_rows($query)==1) {
		...
		if( ($queryunit = mysqli_query($con,$sqlunit)) ){
			...
			if(mysqli_num_rows($queryunit)==1){	
				...
				if($row_inactive_unit['account_type']==1){
				$unit_id = $row_inactive_unit['unit_id']; 

			$sqlinsert ="INSERT INTO unit_head SET title='$title',fname='$fname',lname='$lname',pfnumber='$pfno',phoneno='$pnum',password='$pass',job='$job', email='$email', unit_id='$unit_id'";
						...
						if($queryinert = mysqli_query($con,$sqlinsert)){
								...
								$delsql = "DELETE FROM inactive_unit_rep WHERE email ='$email'";
								 mysqli_query($con,$delsql);
									...
									echo'
									<script>
											window.alert("Account Created, You can Login!");
											window.location.assign("../");
									</script>';
